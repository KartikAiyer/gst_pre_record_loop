...
int main(int argc, char **argv) {
  ...
  if (!prerec_pipeline_create(&tp, "t016-oversize-gop")) FAIL("pipeline create failed");

  guint64 pts = 0; const guint64 dur = 500 * GST_MSECOND;
  /* Single large GOP: key + 15 deltas */
  if (!prerec_push_gop(tp.appsrc, 15, &pts, dur, NULL)) FAIL("oversize gop push failed");

  /* Additional GOP to trigger pruning area */
  if (!prerec_push_gop(tp.appsrc, 2, &pts, dur, NULL)) FAIL("second gop push failed");
