...
int main(int argc, char **argv) {
  ...
  if (!prerec_pipeline_create(&tp, "t017-latency")) FAIL("pipeline create failed");
  ...
  for (guint i = 0; i < NUM_SAMPLES; ++i) {
    ...
    GstQuery *q_before = gst_query_new_custom(GST_QUERY_CUSTOM,
    ...
    guint64 elapsed_ns = (end.tv_sec - start.tv_sec) * 1000000000ULL +
                         (end.tv_nsec - start.tv_nsec);
    
    /* Query stats after push to confirm pruning occurred */
    GstQuery *q_after = gst_query_new_custom(GST_QUERY_CUSTOM,
                                             gst_structure_new_empty("prerec-stats"));
  ...
  if (median_ns > 100 * GST_MSECOND) {
    g_warning("Median latency unusually high (%.3f ms) - possible performance issue",
              median_ns / 1000000.0);
  }
  
  if (p99_ns > 500 * GST_MSECOND) {
    g_warning("99th percentile latency very high (%.3f ms) - check for bottlenecks",
              p99_ns / 1000000.0);
  }
