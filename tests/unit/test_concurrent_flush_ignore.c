...
int main(int argc, char **argv) {
  ...
  if (!prerec_pipeline_create(&tp, "t022-pipeline")) FAIL("pipeline create failed");

  guint64 ts = 0;
  const guint64 per_buf = GST_SECOND; /* 1s */
  guint64 emitted = 0;
  gulong probe_id = prerec_attach_count_probe(tp.pr, &emitted);
  if (!probe_id) FAIL("probe attach failed");
  guint64 baseline = emitted;
  if (baseline != 0) FAIL("expected zero emitted before buffering flush");

  /* Push 3 GOPs (key + 2 deltas) => enough content to observe flush */
  for (int g = 0; g < 3; ++g) {
    if (!prerec_push_gop(tp.appsrc, 2, &ts, per_buf, NULL)) FAIL("push_gop failed");
  }
  ...
  if (after_flush == baseline) FAIL("flush did not emit any buffers");
  ...
  if (after_second_trigger != after_flush) FAIL("second flush trigger caused additional emission");
  ...
  gst_query_unref(q);
  if (queued_buffers != 0) FAIL("expected queue empty after flush");

  /* Send another buffer; since in pass-through it should increment emitted quickly */
  guint64 before_passthrough = emitted;
  if (!prerec_push_gop(tp.appsrc, 0, &ts, per_buf, NULL)) FAIL("push single keyframe failed");
  guint attempts = 0;
  ...
  if (emitted == before_passthrough) FAIL("no passthrough after flush (timeout)");
