...
int main(int argc, char **argv) {
  ...
  if (!prerec_pipeline_create(&tp, "t022-pipeline")) return fail("pipeline create failed");

  guint64 ts = 0;
  const guint64 per_buf = GST_SECOND; /* 1s */
  guint64 emitted = 0;
  gulong probe_id = prerec_attach_count_probe(tp.pr, &emitted);
  if (!probe_id) return fail("probe attach failed");
  guint64 baseline = emitted;
  if (baseline != 0) return fail("expected zero emitted before buffering flush");

  /* Push 3 GOPs (key + 2 deltas) => enough content to observe flush */
  for (int g = 0; g < 3; ++g) {
    if (!prerec_push_gop(tp.appsrc, 2, &ts, per_buf, NULL)) return fail("push_gop failed");
  }
  ...
  if (!gst_element_send_event(tp.pr, flush1)) {
    return fail("first flush event send failed");
  }
  ...
  if (after_flush == baseline) return fail("flush did not emit any buffers");
  ...
  if (after_second_trigger != after_flush) return fail("second flush trigger caused additional emission");
  ...
  guint queued_buffers=999; gst_structure_get_uint(st, "queued-buffers", &queued_buffers);
  gst_query_unref(q);
  if (queued_buffers != 0) return fail("expected queue empty after flush");

  /* Send another buffer; since in pass-through it should increment emitted quickly */
  guint64 before_passthrough = emitted;
  if (!prerec_push_gop(tp.appsrc, 0, &ts, per_buf, NULL)) return fail("push single keyframe failed");
  guint attempts = 0;
