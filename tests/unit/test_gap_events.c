...
int main(int argc, char* argv[]) {
  ...
  if (gap_events_received != 1) {
    ...
    gst_object_unref(src);
    prerec_pipeline_shutdown(&p);
    FAIL("GAP event did not pass through in PASS_THROUGH mode (received=%d expected=1)", gap_events_received);
  }
  ...
  if (gap_events_received != 1) {
    ...
    gst_object_unref(src);
    prerec_pipeline_shutdown(&p);
    FAIL("GAP events not emitted during flush with correct timing (got %d, expected 1)", gap_events_received);
  }
