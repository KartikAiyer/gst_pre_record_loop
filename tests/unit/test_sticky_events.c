...
int main(int argc, char *argv[]) {
  ...
  if (segment_events_received != 1) {
    ...
    gst_object_unref(src);
    prerec_pipeline_shutdown(&p);
    FAIL("SEGMENT event not propagated in BUFFERING mode (received=%d expected=1)", 
         segment_events_received);
  }
  
  /* Verify segment values */
  if (last_segment.format != GST_FORMAT_TIME || last_segment.start != 0 || 
      last_segment.stop != 10 * GST_SECOND) {
    gst_object_unref(sink);
    gst_object_unref(src);
    prerec_pipeline_shutdown(&p);
  ...
  if (segment_events_received < 1) {
    ...
    gst_object_unref(src);
    prerec_pipeline_shutdown(&p);
    FAIL("SEGMENT not re-emitted during flush (received=%d expected>=1)", 
         segment_events_received);
  }
  ...
  if (queried_segment->format != GST_FORMAT_TIME) {
    ...
    gst_object_unref(src);
    prerec_pipeline_shutdown(&p);
    FAIL("Queried SEGMENT has wrong format (format=%d expected=%d)",
         queried_segment->format, GST_FORMAT_TIME);
  }
  gst_event_unref(sticky_segment);
  ...
  gst_event_parse_segment(sticky_segment, &queried_segment);
  if (queried_segment->start != 20 * GST_SECOND || 
      queried_segment->stop != 30 * GST_SECOND ||
      queried_segment->time != 20 * GST_SECOND) {
    gst_event_unref(sticky_segment);
    gst_object_unref(sink);
    gst_object_unref(src);
    prerec_pipeline_shutdown(&p);
    FAIL("Sticky SEGMENT not updated with new values (start=%" G_GUINT64_FORMAT 
