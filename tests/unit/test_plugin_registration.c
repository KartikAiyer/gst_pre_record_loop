...
static gboolean test_caps_acceptance(GstElement *el, const char *caps_str, gboolean should_accept) {
  ...
  if (should_accept) {
    if (!sink_accepts || !src_accepts) {
      g_critical("Element should accept caps '%s' but sink_accepts=%d src_accepts=%d", 
                 caps_str, sink_accepts, src_accepts);
      gst_caps_unref(caps);
    ...
    if (sink_accepts || src_accepts) {
      g_critical("Element should reject caps '%s' but sink_accepts=%d src_accepts=%d", 
                 caps_str, sink_accepts, src_accepts);
      gst_caps_unref(caps);
      gst_caps_unref(sink_caps);
      gst_caps_unref(src_caps);
...
int main(int argc, char *argv[]) {
  ...
  if (!query_caps_sink || !query_caps_src) {
    if (query_caps_sink) gst_caps_unref(query_caps_sink);
    if (query_caps_src) gst_caps_unref(query_caps_src);
    gst_object_unref(sink);
    gst_object_unref(src);
    gst_object_unref(el);
