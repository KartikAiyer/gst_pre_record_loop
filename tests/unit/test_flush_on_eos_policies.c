...
static int test_always_buffering(void) {
  ...
  if (emission_count != 9)
    FAIL("ALWAYS+BUFFERING should drain 9 buffers, got %llu", 
  ...
  if (queued_gops_after != 0 || queued_buffers_after != 0)
    FAIL("ALWAYS+BUFFERING queue should be empty after drain, got %u GOPs, %u buffers",
         queued_gops_after, queued_buffers_after);
...
static int test_never_buffering(void) {
  ...
  if (emission_count != 0)
    FAIL("NEVER+BUFFERING should discard queue (0 emissions), got %llu",
         (unsigned long long)emission_count);
  ...
  if (queued_gops_after != 0 || queued_buffers_after != 0)
    FAIL("NEVER+BUFFERING queue should be empty after EOS, got %u GOPs, %u buffers",
         queued_gops_after, queued_buffers_after);
...
static int test_always_passthrough(void) {
  ...
  if (emission_count != 3)
    FAIL("expected 3 immediate emissions in PASS_THROUGH, got %llu",
         (unsigned long long)emission_count);
  ...
  if (emission_count != before_eos)
    FAIL("ALWAYS+PASS_THROUGH: unexpected emissions at EOS (%llu vs %llu)",
         (unsigned long long)emission_count, (unsigned long long)before_eos);
