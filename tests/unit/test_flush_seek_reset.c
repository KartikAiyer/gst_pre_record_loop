...
downstream_event_probe(GstPad *pad, GstPadProbeInfo *info, gpointer user_data) {
...
int main(int argc, char *argv[]) {
  ...
  gst_pad_add_probe(src, GST_PAD_PROBE_TYPE_EVENT_DOWNSTREAM | GST_PAD_PROBE_TYPE_EVENT_FLUSH,
                    downstream_event_probe, NULL, NULL);
  ...
  if (push_result != GST_FLOW_FLUSHING) {
    ...
    gst_object_unref(sink);
    prerec_pipeline_shutdown(&p);
    FAIL("Buffer push during FLUSH returned %s instead of FLUSHING", 
